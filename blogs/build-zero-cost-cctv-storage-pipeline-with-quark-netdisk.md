# 不花一分钱云存储与内存卡：用一台普通摄像头 + 闲置单板机 + 夸克网盘，自建 24×7 监控存储与回放系统

家里装修或者租房，装个摄像头看家防盗本来是件挺舒心的事。但只要你买过市面上的家用监控（小米、TP-LINK、萤石、华为等），十有八九被厂家的套路恶心过：

1. **硬件便宜，增值服务死贵**：硬件本身一两百块，但买回来才发现，不买他们一年一两百块钱的云存储套餐，只能看实时画面，根本没法回看；
2. **插内存卡？寿命堪忧**：24 小时不间断录 2.5K/4K 视频，普通的 MicroSD (TF) 卡几个月到半年就直接被写入寿命磨损报废；一旦遇上坏人入室，顺手把摄像头或内存卡拔走，所有证据瞬间灰飞烟灭；
3. **NAS 存监控？大炮打蚊子**：开着一套几十上百瓦的大 NAS 24 小时跑监控，噪音、发热加上硬盘电费，成本早够买好几年云服务了。

手头有一台 **TP-LINK TL-IPC44AW（400万像素 2.5K 全彩云台机型，日常裸机价大概在 180~200 元左右）**，桌角又有一块常年通电跑 Linux 的闲置 ARM 开发板（Radxa Cubie A7A，类似树莓派）。

既然硬件本身画质、云台、夜视能力都很硬核，为什么非要掏钱买原厂昂贵的云套餐？
...
- **历史视频全部全自动直传存入 88VIP 附赠的 6TB 夸克网盘（按日期分目录归档，足够连录整整 1 年以上！）**；
...
1. **零成本、零心智负担**：
   - 摄像头不用插卡，不用担心内存卡写坏；
   - 借用淘气值 88VIP 附赠的 6TB 夸克网盘闲置空间，完全不用额外花钱买官方云存；
   - 按每天 15GB 计算，**6TB 空间足以不间断存储整整 400 天（1.1 年）的 2.5K 录像**，远超官方 7 天/30 天滚动的套餐体验；
2. **手机端体验极其丝滑**：
   - 打开手机上的 **夸克网盘 App**；
...
通过把简单的底层工具（FFmpeg Stream Copy）与现代云原生微服务设计结合，我们在一台不到两百块的普通民用监控硬件上，搭建出了一套媲美商业级云存储的稳定安防系统。
